...
function SettingAll() {
  ...
  return (
    ...
    <div>
        ...
        <p className="my-2 ml-8 mr-8 mb-8 mt-8">See information of your account, managing authentication keys, including email, password, or deleting your account. </p>
        <div className="ml-8 flex-row">
          <div className="font-bold flex h-12 items-center">User ID: 
            <div className='pl-8 '>{userId}</div>
          </div>

          <hr className="my-2" /> {/* Horizontal line */}
          <div className="font-bold flex h-12 items-center ">Email: 
            <p className="pl-8 ">{email}</p>
          </div>

          <hr className="my-2" /> {/* Horizontal line */}
          <div className="font-bold flex h-12 items-center justify-between">Password:
          <p className='pl-8'>{showPassword ? password : '**********'}</p>
          <button className="  text-purple-500 hover:brightness-60   font-bold text-base " Icon={EyeSlashIcon} onClick={handlePasswordToggle}>
